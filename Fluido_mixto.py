...
from pathlib import Path # Librería estándar de Python para manipular rutas de archivos y directorios de manera independiente del sistema operativo.
...
#3. CÁLCULO DE LA SOLUCIÓN ANALÍTICA EXACTA (Simbólica Matricial).
    for usar_sommerfeld in [False, True]: # Se usa el for para calcular los valores en base al if y al else de la frontera derecha, ya que se tienen dos casos: Pared rígida (Neumann) y Sommerfeld. Itera dos casos de frontera para obtener soluciones analíticas exactas para cada uno.
        ...
        # Frontera izquierda x = 0 (Presión fija, dirichlet)
...
#4. CREACIÓN DE MALLA Y SUBDOMINIOS EN DOLFINx.
...
#5. BUCLE DE RESOLUCIÓN AUTOMÁTICA CON LINEARPROBLEM
    for nombre_caso, a_form in formas.items(): # El nombre del caso dependerá de la definición del a_form, el cual, dependerá del .items, pues hará las dos iteraciones, una para Neumann y otra para sommerfeld.
        
        # 1. Crear el recipiente vacío para guardar la presión
        ph = fem.Function(V, name="presion_1d")
        
        # 2. Configura y cocina el problema en dos líneas gracias a LinearProblem
        prob = LinearProblem(
        ...
        # Extracción directa y ordenada geométricamente
