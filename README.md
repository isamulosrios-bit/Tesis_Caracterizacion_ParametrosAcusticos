...
* `aire_aire_eulerfluido.py` — Simulación acústica unidimensional/multidimensional considerando un único medio (Aire).
* `aire_agua_mixto.py` — Simulación acústica avanzada en medios multicapa (Aire - Agua) con definición de subdominios e interfaz de acoplamiento.
...
### 1. Modelo de Medio Único (`aire_aire_eulerfluido.py`)
...
### 2. Modelo Acústico Mixto / Interfaz Aire-Agua (`aire_agua_mixto.py`)
...
* **Casos de estudio visualizados:** Las rutinas gráficas permiten contrastar de forma directa el comportamiento del sistema bajo dos configuraciones de frontera en el extremo:
  1. **Pared Rígida:** Reflexión total de la onda sin pérdida hacia el exterior.
